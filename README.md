...
- **Country flags** — show a small flag on graph nodes whose cluster metadata contains a recognized `meta.country` value. Flags are enabled by default, can be disabled from the toolbar, and the preference is saved locally. Shape-mode labels receive the same optional flag prefix.
